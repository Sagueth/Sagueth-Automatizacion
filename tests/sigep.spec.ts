...
const { verificarSoportesVarios, verificarEducacionFormal, verificarExperienciaLaboral, verificarExperienciaLaboralDocente, verificarDocumentosAdicionales, verificarEducacionTrabajo, verificarIdiomas } = require('../utils/gemini');
const { generarReportePDF } = require('../utils/pdf');
...
// Leer numero de dcto del excel "base.xlsx"
function obtenerDocumentos() {
    //Excel completo
    //const workbook = XLSX.readFile('./todoFuncionarios/base.xlsx'); 
    //Excel Alvaro (pruebas)
    const workbook = XLSX.readFile('./funcionarios/base.xlsx'); 
    ...
    while (sheet[`A${row}`]) {
        documentos.push(sheet[`A${row}`].v.toString()); // Convertir a string y agregar a la lista
        row++;
    }
...
test('Ejecutar prueba con detección de sesión expirada', async ({ page, context }) => {
    ...
    // Consulta de hojas de vida del Excel
    for (const documento of documentos) {
        ...
        while (!intentoExitoso) {
            ...
            try {
                ...
                break; // si no hay botón, no vale la pena seguir intentando este documento
            }
            ...
            try {
                ...
                if (!datos.includes('Tipo de Documento:') || !datos.match(/\d{1,}/)) {
                    ...
                    continue; // reintentar el mismo documento
                }
